...
arriveReq.onreadystatechange = function () {
    if (arriveReq.readyState === 4) {
        if (arriveReq.status === 200) {
            ...
            if (arriveInfo) {
                busTarget = document.querySelector('.parkingList')
                busTemp = arrivingInfo.template;
                busTemp2 = Handlebars.compile(busTemp);
                busTarget.innerHTML += busTemp2(arriveInfo.parking);
