...
//getting arriving info from arrivingList.txt with AJAX
var arriveReq=new XMLHttpRequest;
arriveReq.open('GET','/source/templates/arrivingLists.text');
arriveReq.onreadystatechange=function(){
    if (arriveReq.readyState === 4) {
        if (arriveReq.status === 200) {
            arriveInfo = arriveReq.responseText;
            arriveinfo=JSON.parse(arriveInfo)
